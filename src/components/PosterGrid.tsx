...
import type { ViewMode } from "./ArchiveControls";
...
export function PosterGrid({ posters, onOpen, view = "grid" }: PosterGridProps) {
  return (
      ...
      <div className="relative z-[2] mx-auto max-w-[1600px] px-3 py-6 sm:px-4 sm:py-8">
        {view === "grid" ? (
          ...
          <motion.div layout className="flex flex-col gap-2.5 sm:gap-3">
            <AnimatePresence mode="popLayout">
              {posters.map((poster) => (
                <PosterRow key={poster.id} poster={poster} onOpen={onOpen} />
              ))}
            </AnimatePresence>
          </motion.div>
        )}
